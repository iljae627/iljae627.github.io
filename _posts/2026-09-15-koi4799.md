...
문제는 **2021 KOI 2차 고등부 2번** (또는 중등부 3번)에 출제된 문제다.
...
<!-- 이미지 자리: 예제 그래프의 정점 가중치 표현 예시 그림 -->
```mermaid
graph TD
    Node1["(1번 정점) x₁ = k"]
    Node2["(2번 정점) x₂ = -k + 5"]
...
* **선형 탐색 최적화:** DFS 1회 ($O(N+M)$) + 정렬 1회 ($O(N \log N)$) 로 시간 복잡도를 $O(N \log N + M)$ 수준으로 유지하여 250ms 시간 제한을 여유 있게 통과.
...
<!-- 이미지 자리: 정올 사이트에 제출하여 "정답입니다" 및 250ms 이하 실행 시간이 나온 채점 결과 화면 사진 -->
