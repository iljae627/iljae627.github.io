...
Exercise 2 명령과 추가 명령을 구현하고 테스트까지 마쳤다. 다중 스레드와 `eval()`의 부작용은 남아 있으므로 범용 디버거라기보다 Python 실행 모델을 확인하기 위한 툴이라고 보는게 맞는듯하다. 일단은 여기서 마무리 하지만, 추후 기능 확장 및 명령 구현 해볼 예정이다.
