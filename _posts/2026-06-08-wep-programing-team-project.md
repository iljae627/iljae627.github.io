---
title: "InfoSec Hub 개발 기록 (1): 중간고사 마일스톤 - 시스템 설계부터 프론트엔드 인프라 구축까지"
...
# InfoSec Hub: 정보보호 지식 아카이브 플랫폼 제작기 (중간고사 마일스톤)


## 1. 프로젝트 아키텍처 기획 및 팀별 도메인 분업 구조


중부대학교 정보보호학 전공 과정에서 학습하는 지식은 공격(Offensive), 방어(Defensive), 그리고 침해사고 분석(Analysis)에 이르기까지 그 범위가 매우 방대합니다. 
...
중간고사 마일스톤의 핵심 목표는 **"기말고사의 동적 비즈니스 로직이 어떤 화면에서도 완벽하게 구동될 수 있도록 확장성 있는 프론트엔드 인프라와 정적 아카이브 뼈대를 완성하는 것"**이었습니다. 이를 위해 플랫폼을 구성하는 레이아웃과 서브페이지 아키텍처를 완전히 통일하고, 5대 보안 도메인을 설정하여 각 팀원의 강점에 맞춰 심화 마크업을 진행했습니다.
...
웹 표준과 검색엔진 최적화(SEO), 그리고 스크린 리더 접근성을 보장하기 위해 전체 구조는 구조적 의미를 지닌 HTML5 시맨틱 태그로만 쪼개어 배정했습니다. 
...
우리가 매일 러닝을 하며 기초 체력을 다지듯 완성한 InfoSec Hub 플랫폼은 지식의 파편화를 막는 훌륭한 허브가 되었습니다. 앞으로는 클라우드 인프라를 연동하여 더욱 진화된 보안 관제 아카이브로 발전시켜 나갈 것입니다.